...
class PersonProfiles:
    ...
    def smooth_pose_xyz(self, person_id: int,
                        x: float, y: float, z: float,
                        timestamp_s: float) -> Tuple[float, float, float]:
        """EMA-smooth Cartesian REP-103 position (x=fwd, y=left, z=up).
        Replaces smooth_display_values: smoothing Cartesian avoids spherical
        wraparound artefacts and is correct after the REP-103 conversion.
        """
        val = np.array([x, y, z], dtype=np.float64)
    ...
    def compute_velocity(self, person_id: int,
        ...
        # ObjectVelocity uses X=forward, Y=left (REP-103), Z=up, so axes are remapped here.
...
def compute_3d_pose(pixel_coords, kp3d_normalized, joint_scores,
                    ...
                    depth_hint_m:       Optional[float] = None):
    ...
    if depth_hint_m is not None and depth_hint_m > JOINT_DEPTH_MIN_M:
        z_root = float(depth_hint_m)
    else:
...
def compute_gaze_ray_3d(joints_meters: np.ndarray,
                        joint_scores:  np.ndarray) -> Optional["ObjectGaze"]:
    """Return ObjectGaze (REP-103) from ear-midpoint to nose, or None.
    Requires nose + both ears above MIN_KEYPOINT_CONFIDENCE.
    az_deg: horizontal from forward, positive=right.
    el_deg: vertical above horizon, positive=up.
    """
    ...
    if length < 1e-6:
        return None
    unit_dir = (gaze_vec / length).astype(np.float32)
    az_deg, el_deg = gaze_angles_3d(unit_dir)
    return ObjectGaze(
        origin    = joints_meters[JOINT_NOSE].copy(),
        direction = unit_dir,
        az_deg    = az_deg,
        el_deg    = el_deg,
    )
...
def _fit_ground_plane() -> None:
    ...
    if normal[1] > 0:
        normal = -normal
    if abs(float(normal[1])) < GROUND_PLANE_NORMAL_MIN_Z:
        return
...
# ── REP-103 / camera-space helpers ───────────────────────────────────────────
def _cam_to_rep103_xyz(v: np.ndarray) -> np.ndarray:
    """Remap camera axes (X=right, Y=down, Z=fwd) → REP-103 (X=fwd, Y=left, Z=up).
    ...
    out = np.empty_like(v)
    out[:, 0] =  v[:, 2]  # x_rep = z_cam
    out[:, 1] = -v[:, 0]  # y_rep = -x_cam
    out[:, 2] = -v[:, 1]  # z_rep = -y_cam
    return out


def _cam_yaw(centroid_3d: np.ndarray) -> float:
    """REP-103 yaw: 0=forward, positive=left [rad].
    atan2(-cam_x, cam_z) — identical to atan2(rep_y, rep_x).
    """
    return math.atan2(-float(centroid_3d[0]), float(centroid_3d[2]))


def _visible_score(sc: np.ndarray, jv: np.ndarray) -> float:
    """Mean score of visible joints; 0.0 if none visible."""
    vis = sc[jv]
    return float(vis.mean()) if vis.size > 0 else 0.0


def _make_label(h_m: float, vel: ObjectVelocity) -> ObjectLabel:
    """Build human-readable ObjectLabel from height and velocity."""
    spd_kmh = math.sqrt(vel.vx**2 + vel.vy**2) * 3.6
    return ObjectLabel(
        text  = f"H:{h_m*100:.0f}cm spd:{spd_kmh:.1f}km/h",
        value = round(spd_kmh, 2),
    )
...
# ── Detection helpers ────────────────────────────────────────────────────────
def detect_persons(kp3d_all, scores_all, kp2d_all,
                   K: np.ndarray, D: np.ndarray) -> list:
    """Filter and undistort raw model outputs.
    Returns list of (bbox, conf, px, k3, sc) — one entry per accepted detection.
    Applies MIN_VISIBLE_KEYPOINTS and MIN_DETECTION_MEAN_CONFIDENCE gates.
    """
    result = []
    for i in range(len(kp3d_all)):
        ...
        result.append(([x1, y1, x2, y2], det_conf, px, k3, sc))
    return result


def enrich_with_skeleton(msg: "ObjectMsg",
                         jm_cam: np.ndarray,
                         sc:     np.ndarray) -> None:
    """Fill msg.keypoints_3d, msg.joint_scores, msg.score, msg.gaze in-place.
    keypoints_3d are stored in REP-103 [m]; gaze from compute_gaze_ray_3d().
    """
    jv              = sc > MIN_KEYPOINT_CONFIDENCE
    msg.joint_scores  = sc
    msg.keypoints_3d  = _cam_to_rep103_xyz(jm_cam)
    msg.score         = _visible_score(sc, jv)
    gaze              = compute_gaze_ray_3d(jm_cam, sc)
    msg.gaze          = gaze
    msg.gaze_valid    = gaze is not None
...
def process_frame(bgr_frame, timestamp_s: float = 0.0) -> List[PersonData]:
    """Run inference + tracking for one frame; return per-person results in REP-103.
    All ObjectMsg fields are populated (pose, size, velocity, label, gaze, keypoints).
    """
    ...
    for pid, det_idx in _tracker.update(bboxes, confs):
        ...
        # Camera (X=right,Y=down,Z=fwd) → REP-103 (X=fwd,Y=left,Z=up), smooth
        cx_r, cy_r, cz_r = _cam_to_rep103_xyz(centroid_3d)
        sx, sy, sz        = _profiles.smooth_pose_xyz(pid, cx_r, cy_r, cz_r, timestamp_s)

        velocity = _profiles.compute_velocity(pid, centroid_3d, timestamp_s)
        sm_h     = _profiles.get_published_height(pid, timestamp_s)

        pose  = ObjectPose(x=sx, y=sy, z=sz, yaw=_cam_yaw(centroid_3d))
        size  = ObjectSize(h=round(sm_h, 3))
        label = _make_label(sm_h, velocity)
        msg   = ObjectMsg(id=pid, pose=pose, size=size,
                          velocity=velocity, label=label)
        enrich_with_skeleton(msg, jm, sc)

        jv = sc > MIN_KEYPOINT_CONFIDENCE
        results.append(PersonData(object_msg=msg, pixel_coords=px, joint_visible=jv))

        # Ground plane: use local jm (camera space) directly — no re-read from p
        if do_ground:
            hip_vis = [j for j in [JOINT_LEFT_HIP, JOINT_RIGHT_HIP] if jv[j]]
            if hip_vis:
                h_ref  = _profiles.get_height_reference(pid)
                gnd_pt = jm[hip_vis].mean(axis=0).copy()
                gnd_pt[1] += (1.0 - VISIBLE_BODY_RATIO_HIPS) * h_ref  # Y+ = down
